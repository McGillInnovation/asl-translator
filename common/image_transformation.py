...
def make_background_black(frame):
    ...
    # Tuned parameters to match the object color of the input images...
    lower_boundary = np.array([0, 40, 30], dtype="uint8")
    upper_boundary = np.array([43, 255, 254], dtype="uint8")
    object_mask = cv2.inRange(frame, lower_boundary, upper_boundary)
    ...
    object_mask2 = cv2.inRange(frame, lower_boundary, upper_boundary)

    # Combine the effect of both the masks to create the final frame.
    object_mask = cv2.addWeighted(object_mask, 0.5, object_mask2, 0.5, 0.0)
    # Blur the mask to help remove noise.
    # object_mask = cv2.medianBlur(object_mask, 5)
    frame_object = cv2.bitwise_and(frame, frame, mask=object_mask)
    frame = cv2.addWeighted(frame, 1.5, frame_object, -0.5, 0)
    frame_object = cv2.bitwise_and(frame, frame, mask=object_mask)
...
def make_object_white(frame):
    """
    Makes the object color white.
    """
    print("Making object white...")
    ...
    for i in range(height):
        for j in range(width):
            if frame[i][j] > threshold:
                # Setting the object tone to be white.
                frame[i][j] = 255
            else:
